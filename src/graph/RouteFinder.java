...
//	}
	...
	private List<Path> findPaths(Node start, Node end, OptimizationCriteria criteria) {
	    ...
	    while (!queue.isEmpty()) {
	        ...
	        // Get edges using the most basic method - compatible with all GraphStream versions
	        for (int i = 0; i < currentNode.getDegree(); i++) {
	            ...
	            PathInfo newPathInfo = new PathInfo(newPath, newTotalTime, newTotalCost, newTotalTransfers, newTotalWeight);
	    ...
	    return bestPaths.getOrDefault(end, Collections.emptyList()).stream()
	            .map(PathInfo::getPath)
