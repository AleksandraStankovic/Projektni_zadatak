 ...
 * 
 * <p>
 * This panel embeds a GraphStream SwingViewer to display the graph, allows updating the graph,
 * and supports highlighting a path of cities by visually distinguishing nodes and edges along the path.
 * </p>
 * 
 * <p>
 * Node and edge styles can be dynamically updated to show the currently highlighted path, while
 * all other nodes and edges are displayed in default styles.
 * </p>
 */
public class CityGraphPanel extends JPanel {
    ...
    /**
     * Highlights a path of cities in the graph by updating the visual styles of nodes and edges.
     * 
     * <p>
     * All edges and nodes are first reset to default "cityEdge" and "cityNode" styles. 
     * Then, for each consecutive pair of cities in the given path, the corresponding edge 
     * and its source and target nodes are styled as "highlightedEdge" and "highlightedCity".
     * </p>
     * 
     * <p>
    ...
    public void highlightCityPath(List<String> cityPath) {
        ...
        for (int i = 0; i < cityPath.size() - 1; i++) {
            ...
            if (edge == null) edge = graph.getEdge(to + "-" + from);
            if (edge != null) {
