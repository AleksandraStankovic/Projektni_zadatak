...
public class MainFrame extends JFrame {
    ...
    public MainFrame(TransportDataGenerator.TransportData data) {
        ...
        gbc.gridx = 0; topFormPanel.add(new JLabel("Polazište:"), gbc);
        gbc.gridx = 1; topFormPanel.add(cbPolaziste, gbc);
        gbc.gridx = 2; topFormPanel.add(new JLabel("Odredište:"), gbc);
        gbc.gridx = 3; topFormPanel.add(cbOdrediste, gbc);
        gbc.gridx = 4; topFormPanel.add(new JLabel("Kriterijum:"), gbc);
        gbc.gridx = 5; topFormPanel.add(cbKriterijum, gbc);
        gbc.gridx = 6; topFormPanel.add(btnPronadji, gbc);
        ...
        btnPronadji.addActionListener(e -> {
            ...
            if (bestRoute == null) {
                ...
                // City graph
                List<String> cityPath = controller.extractCityPath(bestRoute); // must define before usage
                CityGraph cityGraph = new CityGraph(data);
                cityGraph.highlightPath(cityPath); // mark optimal path
                CityGraphFrame cityFrame = new CityGraphFrame(cityGraph);
                cityFrame.getGraphPanel().highlightCityPath(cityPath); // refresh panel
        ...
        btnKupi.addActionListener(e -> {
            if (lastBestRoute == null) {
                JOptionPane.showMessageDialog(this, "Prvo pronađite rutu za koju želite kupiti kartu.", "Greška", JOptionPane.ERROR_MESSAGE);
                return;
            }
